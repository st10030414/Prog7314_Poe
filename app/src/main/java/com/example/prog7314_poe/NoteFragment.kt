...
class NoteFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        aiSpinner = view.findViewById(R.id.ai_prompt)
        ...
        val saveAndBack = {
            saveNote(title.text.toString(), content.text.toString(), tagSpinner.selectedItem.toString())
    ...
    private fun saveNote(titleText: String, contentText: String, tagText: String) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            if (noteId != null) {
                ...
                existing?.let {
                    val updatedNote = it.copy(title = titleText, content = contentText, tags = tagText)
                    noteDao.update(updatedNote)
                }
            } else {
                // Insert new note
                val newNote = Note(title = titleText, content = contentText, tags = tagText)
    ...
    private fun applyFontSizes() {
        // EditTexts use Utility font size
        title.setTextSize(TypedValue.COMPLEX_UNIT_PX, Utility.getFontDimen(requireContext(), "body"))
        content.setTextSize(TypedValue.COMPLEX_UNIT_PX, Utility.getFontDimen(requireContext(), "body"))

        // Apply hard-coded size to spinner adapters
        tagSpinner.adapter = createSpinnerAdapter(tagSpinner, R.array.tag_prompts)
        aiSpinner.adapter = createSpinnerAdapter(aiSpinner, R.array.ai_prompts)
    }
